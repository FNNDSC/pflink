...
def test_create_workflow(test_app, monkeypatch, sample_workflow_request, sample_workflow_entry):
    ...
    response = test_app.post("api/v1/workflow", data=json.dumps(sample_workflow_request))
    assert response.status_code == 200
    assert response.json() == sample_workflow_entry["response"]


@pytest.mark.mocktest
def test_get_all_workflows(test_app, monkeypatch, sample_workflow_entry):
